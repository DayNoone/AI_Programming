...
def draw_board(variables, graph, finished):
    ...
    for i in variables:
        for j in i.neighbor:
            if (variables[j].colorid is not None or i.colorid is not None): # only draw lines to variables with colors
                coordinates1 = ((i.xPos + x_offset) * multiplier + coffset, (i.yPos + y_offset) * multiplier + coffset)
                coordinates2 = ((variables[j].xPos + x_offset) * multiplier + coffset, (variables[j].yPos + y_offset) * multiplier + coffset)
