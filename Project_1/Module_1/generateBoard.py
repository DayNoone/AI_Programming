...
def getBoardInfoFromInput():
	...
	while 1:
		...
		if prompt == 'y':
			...
			break
	return Board(generateBoard(int(dimX), int(dimY), int(startX), int(startY), int(goalX), int(goalY), barriers),
	             (int(startX), int(startY)), (int(goalX), int(goalY)))


def readBoard(boardNo):
	if os.name == 'nt':
		file = open('boards\\' + fileBoards[boardNo], 'r')
	else:
		file = open('boards/' + fileBoards[boardNo], 'r')
	...
	for line in file.readlines():
		...
		newX = []
		for char in x:
			newX.append(int(char))
		boardInfo.append(newX)
	print boardInfo

	barriers = []
	for i in range(2, len(boardInfo)):
		barriers.append(boardInfo[i])

	file.close()

	return Board(generateBoard(boardInfo[0][0], boardInfo[0][1],
	                           boardInfo[1][0], boardInfo[1][1],
	                           boardInfo[2][0], boardInfo[2][1],
	                           barriers),
	             (boardInfo[1][0], boardInfo[1][1]),
	             (boardInfo[1][2], boardInfo[1][3]))


def getBoardFromFile(boardIndex):
	return readBoard(boardIndex)


board_test = Board(generateBoard(6, 6, 1, 0, 5, 5, [[3, 2, 2, 2], [0, 3, 1, 3], [2, 0, 4, 2], [2, 5, 2, 1]]), (1, 0),
                   (5, 5))
